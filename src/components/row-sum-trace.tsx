...
import { IZERO_COLOR } from "@/lib/stxm/region-colors";
import { computeRowSums, rowSumToTraceX, rowSumTraceLimits } from "@/lib/stxm/plot-scale";

export const ROW_SUM_TRACE_WIDTH = 52;

type RowSumTraceProps = {
  image: number[][];
  height: number;
  width?: number;
  qaxisPoints: number[];
...
export function RowSumTrace({
  image,
  height,
  width = ROW_SUM_TRACE_WIDTH,
  qaxisPoints,
  sampleMin,
  yToPx,
  izero,
  ...
  useEffect(() => {
    ...
    const izeroTop = yToPx(izero.izero_hi, height);
    const izeroBottom = yToPx(izero.izero_lo, height);
